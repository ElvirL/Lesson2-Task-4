...
void print_file(vector<int>& ATM){
    ...
    cout << "Общая сумма в банкомате: " << get_sum_money_in_ATM(ATM) << endl;
    cout << endl;
}
...
void give_money_to_client(vector<int>& ATM){
    ...
    if (get_sum_money_in_ATM(ATM) == 0){
        cout << "В банкомате отсутстуют деньги" << endl;
    }
    else {
        ...
        while (sum > get_sum_money_in_ATM(ATM)){
            cout << "В банкомате недостаточно денег. Введите сумму меньшую или равную " << get_sum_money_in_ATM(ATM) << endl;
            cin >> sum;
        }

        print_file(ATM);

        cout << "Выдано купюр: " << endl;
